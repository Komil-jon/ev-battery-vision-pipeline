...
\title{Cross-Facility Generalisation of Vision-Based\\Battery Module Detection for Robotic\\Electric Vehicle Battery Disassembly}
...
damaged modules, matching a supervised classifier trained on scarce damaged data. We
...
generalisation, foundation models, anomaly detection, recycling automation
\end{IEEEkeywords}
...
We investigate this directly and make five contributions:
...
\midrule
ev-battery-component-detection & 1,045 & multi-manufacturer \\
ev-battery-components & 680 & Audi/BMW/Chevrolet/Hyundai/Porsche \\
last\_exp4 & 483 & multi-manufacturer \\
ev-battery-pack (single facility) & 435 & laboratory rig \\
battery\_comp & 280 & Nissan Leaf \\
ue\_d1\_defect\_detection & 237 & module close-ups \\
final\_mobilenet\_results & 216 & multi-manufacturer \\
automated-disassembly & 117 & disassembly cell \\
bmw\_i3 & 111 & BMW i3 \\
ue\_rav4\_module & 96 & Toyota RAV4 \\
...
Detection localises \texttt{module} and \texttt{busbar} instances. Each detected
module crop is then passed to a condition-assessment stage that outputs a reuse
grade: A (reusable), B (manual review), C (recycle), thresholded on the predicted
...
YOLO11n (multi-source) & 0.410 & 0.256 & 0.547 & 0.304 \\
...
are effectively tied: overall 0.558 vs 0.544, and module 0.771 vs 0.774
(Table~\ref{tab:nooutlier}). The DINOv2 backbone confers \emph{no} advantage on
conventionally annotated packs.

\begin{table}[t]
\caption{Consensus-convention subset (177 images): the models are tied}
\label{tab:nooutlier}
\centering
\small
\begin{tabular}{lccc}
\toprule
Model & overall & module & busbar \\
\midrule
YOLO11n (multi-source) & 0.544 & \textbf{0.774} & 0.344 \\
RF-DETR (DINOv2) & \textbf{0.558} & 0.771 & \textbf{0.366} \\
\bottomrule
\end{tabular}
\end{table}
...
Fig.~\ref{fig:persource} shows per-source module mAP@50 for the multi-source
detector. Performance is excellent on most packs -- 0.995, 0.910, 0.873, 0.749 --
and collapses to 0.043 on exactly one source.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_persource.pdf}
\caption{Per-source module mAP@50. The 0.043 outlier is not a visually harder pack;
it uses a divergent definition of ``module'' (small internal sub-components in macro
views) that disagrees with the other eight sources.}
\label{fig:persource}
\end{figure}
...
Fig.~\ref{fig:perclass} summarises per-class behaviour. Busbar remains the weaker
class for every model (0.30--0.37). We traced a probable cause: 61\% of busbar
instances in the test set come from a single source, and only four of eight sources
annotate busbars at all. Busbar therefore suffers the same single-source annotation
coverage problem that module detection suffers from convention conflict.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_perclass.pdf}
\caption{Per-class cross-facility mAP@50. Busbar is consistently weaker, tracking its
sparse and single-source-dominated annotation coverage.}
\label{fig:perclass}
\end{figure}
...
\begin{tabular}{lccc}
\toprule
Method & Damaged & Acc. & Damaged training \\
 & recall & & examples used \\
\midrule
Supervised ResNet18 & 0.857 & 0.792 & 80 \\
DINOv2 linear probe & -- & 0.792 & 80 \\
Good-only anomaly (ours) & \textbf{0.857} & -- & \textbf{0} \\
\bottomrule
\end{tabular}
\end{table}
...
edge deployment on a disassembly cell.
...
Our claims are about perception only. We have not integrated with a manipulator, and
as RAPID's 57\% one-shot grasp rate at 0.9757 mAP@50 shows, detection accuracy does
not translate directly into manipulation success. Converting our 2D detections into
grasp poses requires depth sensing and hand-eye calibration, which we have not
performed.
...
Future work is to convert instance masks into grasp poses using depth sensing, and to
close the label loop by combining worker annotations with electrical measurements
taken on the line, which would supply the damage labels that public data lacks.
