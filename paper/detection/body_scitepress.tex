...
detection transformer initialised from the published RF-DETR-Nano checkpoint, whose
backbone derives from self-supervised DINOv2 pretraining \citep{dinov2}, fine-tuned
end to end on the same data. All 30.6\,M parameters are updated; the backbone is not
frozen. The resolution and epoch budget are the
...
a self-supervised pretrained backbone does not detect ordinary modules more
...
excluding it. A self-supervised pretrained backbone was shown to supply robustness to
