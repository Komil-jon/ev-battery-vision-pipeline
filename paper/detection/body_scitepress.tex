...
1.000 to 0.097, and inspection confirms that the source on which accuracy collapses
annotates small internal sub-components as modules while every other source annotates
pack-level modules. The detector learned the majority
...
\noindent Zang et al. surveyed robotic EV battery
disassembly and concluded that the process remains predominantly manual, with
perception and task uncertainty identified as the principal obstacles \cite{zang2024robotic}. Wegener et al. demonstrated that pack designs vary sufficiently
between manufacturers that fixed automation does not transfer, which is the
practical reason perception must generalise \cite{wegener2015}. However, that study addressed
...
components. Li et al. proposed a screw detection method
combining Faster R-CNN with rotation edge similarity, reporting improved
localisation of the screw axis \cite{screwrcnn2021}. However, the method was validated on a single
collection of laboratory imagery. Li et al. developed a
YOLOX-family detector for active screw detection in EV battery disassembly using
1,719 hand-annotated images \cite{screwbatt2023}. However, the dataset was not released and accuracy
was reported only in domain. Naseri et al.
...
transfer to battery imagery \cite{screwcompare2026}.
...
Penquitt et al. developed Rechecked, a modular framework combining automatic
...
found to be missing or inaccurate \cite{penquitt2026}. However, the framework corrects errors relative
...
Abou Akar et al. investigated multi-source training for industrial object
...
generalisation \cite{abouakar2026}. However, the sources compared in that study share a single
...
because a divergent source inflates both of the latter and thereby partially masks
itself, so the procedure stays sensitive when that source is large.
...
fence, an order of magnitude below the consensus band. Figure~\ref{fig:examples}
shows the diagnosis directly: it annotates small internal sub-components as modules,
whereas every other source annotates pack-level modules.

\begin{figure}[t]
\centering
\includegraphics[width=0.98\columnwidth]{figures/fig_convention_examples.pdf}
\caption{Two frames from the flagged source with their released annotations. The
source annotates busbars separately and correctly (yellow), so the boxes it labels
\texttt{module} (red) are not mislabelled busbars: they mark small internal
sub-components, while the pack-level modules that every other source annotates are
left unmarked. Median normalised module scale is 0.218 and 0.169 here, against 0.14
for this source overall and 0.17 to 0.54 across the consensus sources.}
\label{fig:examples}
\end{figure}
...
Secondly, curation and the choice of pretraining address the same failure mode by
different means, and they are complementary rather than alternative. Excluding the
divergent source removes it from the aggregate entirely, whereas self-supervised
pretraining raises accuracy on it from 0.097 to 0.337 without removing it. A divergent
source that has not been identified is therefore still handled more gracefully by the
pretrained model, which argues for applying both.

Thirdly, and least expected, the same annotation disagreement that distorts reported
