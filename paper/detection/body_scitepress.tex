...
\cite{eccrm,harper2019}, and around 170\,GWh of batteries are projected to reach end of
life and become available for recycling in Europe by 2035 \cite{te2024}. Disassembly of EoL packs nevertheless remains predominantly manual. The
...
directed at automating it \cite{zang2024robotic,tan2025screws}.
...
about what the target class denotes. Per-source module accuracy spans
1.000 to 0.097, and the source on which accuracy collapses annotates at a
systematically finer granularity than the rest, with a median module box roughly 60\% the
linear size of those in the other sources. The detector learned the
majority convention and applied it consistently, so the collapse constitutes a
disagreement about the target rather than a failure of perception.

\begin{figure}[t]
\centering
\includegraphics[width=0.96\columnwidth]{figures/fig_overview.pdf}
\caption{Overview of the two screening procedures. Both take a corpus assembled from
independent sources and return the sources whose annotation convention diverges from
the majority. The model-based screen scores each source with a detector trained on the
merged corpus; the training-free screen reads the annotation files alone and needs no
training run.}
\label{fig:overview}
\end{figure}
...
\noindent Zang et al. surveyed robotic EV battery disassembly and identified
perception and task uncertainty as the principal obstacles to a process that remains
predominantly manual \cite{zang2024robotic}. Wegener et al. showed that pack designs vary enough
between manufacturers that fixed automation does not transfer, which is the practical
reason perception must generalise \cite{wegener2015}. However, that study addressed disassembly
sequencing and evaluated no perception component.

Most published perception work for this application targets fasteners rather than
components. Li et al. combined Faster R-CNN with rotation edge similarity for screw detection but
validated it on a single collection of laboratory imagery \cite{screwrcnn2021}. Li et al. developed
a YOLOX-family detector using 1,719 hand-annotated images, which were not released, and
reported accuracy only in domain \cite{screwbatt2023}. Naseri et al. compared YOLOv11 against Faster
R-CNN and observed that the field is hampered by a lack of public datasets; that study
resorts to laptop screws as a proxy domain, which limits what transfers to battery
imagery \cite{screwcompare2026}.

Module-level and busbar-level perception is comparatively under-explored. Tan et al. developed a collaborative robot that unfastens and
collects screws by fusing vision with force and torque sensing, reporting above
90\% unfastening efficiency \cite{tan2025screws}. However, the vision component operates on a single
fixed pack geometry, so the reported figure does not characterise transfer.
Gerlitz et al. applied instance segmentation with point-cloud
registration in order to grasp busbars on an industrial module \cite{cvmodule2023}. However, the
...
Recht et al. demonstrated that classifiers which appear strong
on the original ImageNet test split lose accuracy on a replication set collected
by the same protocol \cite{recht2019}. However, that study varied the sampling procedure while
holding the label taxonomy fixed. Koh et al. assembled the
...
such shift \cite{koh2021wilds}. However, WILDS likewise holds the annotation protocol constant within
...
Northcutt et al. demonstrated that pervasive label errors in
established test sets destabilise benchmark rankings, and estimated an average
label error rate of 3.4\% across ten widely used datasets \cite{northcutt2021}. However, that work
...
Penquitt et al. developed Rechecked, which combines automatic label-error detection
with crowd-sourced verification, and found 18\% of the KITTI pedestrian ground truth
missing or inaccurate \cite{penquitt2026}. However, it corrects errors relative to a standard agreed
within one dataset. The failure characterised here is different: the sources do not
disagree about individual boxes, they disagree about what the class denotes, so no
correction operating within a single convention can reconcile them.
Section~\ref{sec:negative} reports an attempt at exactly that, and its failure.

Abou Akar et al. trained industrial detectors on real, rendered and generatively
synthesised data, and showed that the composition of the mixture materially affects
generalisation \cite{abouakar2026}. However, those sources share one annotation standard by
construction, since synthetic data carries its ground truth. The corpus here is
multi-source in a stricter sense: each source carries an independently authored
standard.

\subsection{Self-Supervised Representations for Detection}

\noindent Self-supervised backbones such as DINOv2 \cite{dinov2} learn features
that transfer without task-specific fine-tuning, and detection transformers
\cite{carion2020detr} initialised from them \cite{rfdetr} are reported to generalise better than
...
\noindent Open-vocabulary detectors such as YOLO-World \cite{yoloworld} and Grounding
DINO \cite{groundingdino} localise objects from free-text prompts and have been
proposed as a route to annotation-free deployment. However, their accuracy depends on
the target concept appearing in the vision-language pre-training corpus; the behaviour
of both on battery internals is reported in Section~\ref{sec:negative}.

Benchmark design for industrial vision has precedent. Bergmann et al. introduced
MVTec AD, showing that a carefully constructed benchmark can redirect a sub-field,
though it was collected by one group under controlled conditions and exhibits no
inter-source annotation disagreement \cite{bergmann2019mvtec}. Zhou et al. surveyed domain
generalisation, but that taxonomy presumes a fixed label space across domains, which is
the assumption shown here to fail on assembled corpora \cite{zhou2022dg}.
...
\hline
ev-battery-comp-det. & 1,045 & multi-manuf. \\
ev-battery-components & 680 & Audi/BMW/Chevy \\
last\_exp4 & 483 & multi-manuf. \\
ev-battery-pack & 435 & single lab rig \\
battery\_comp, ue\_d1 & 517 & Nissan Leaf, close-ups \\
final\_mobilenet & 216 & multi-manuf. \\
automated-disassem. & 117 & disassembly cell \\
bmw\_i3, ue\_rav4 & 207 & BMW i3, Toyota RAV4 \\
Others (3 sources) & 725 & mixed \\
\hline
\end{tabular}
\end{table}
...
overlap the training data. All 4,440 candidate images were therefore compared by difference hashing (dHash) at a
Hamming distance threshold of 6, selected because values below 4 admitted rotated
duplicates and values above 8 rejected distinct frames of the same pack. Fifteen
same-source near-duplicates were removed, and a cross-source pass at a looser
threshold returned zero duplicates, confirming the sources to be genuinely independent
collections rather than republications of one another.
...
Two findings changed how the corpus was treated. Firstly, the blur and greyscale flags
are largely false alarms: the lowest-scoring images are smooth closed pack lids, for
which a Laplacian-variance metric reads flat metal as blurred, and the greyscale images
come from two monochrome sources. Both add useful variation, so deleting on the metric
alone would degrade the corpus.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{figures/fig_scaledist.pdf}
\caption{Distribution of normalised module box scale for the flagged source against
all other sources. Dashed lines mark the medians. The flagged source annotates at a
systematically finer granularity, though the two distributions overlap and a
substantial share of its boxes falls inside the range of the others.}
\label{fig:scaledist}
\end{figure}

The audit also established that 16,945 of the 19,522 annotations are polygon masks with
between 5 and 151 vertices rather than axis-aligned boxes. These are converted to
enclosing boxes for training and evaluation. It should be noted that an early version
of the evaluation pipeline silently discarded every polygon-format label, trained on
20\% of the annotations, and produced an inflated score that appeared plausible.
Ground-truth instance counts should be reconciled across pipelines as a matter of
routine.

\subsection{Evaluation Sets}

\noindent Two evaluation sets are used and are kept distinct throughout. A
cross-facility benchmark of 225 images containing 780 instances was drawn from all
sources and held out entirely from training; it supports the single-source comparison
of Section~\ref{sec:res-gap} and nothing else. The screening procedures of
Sections~\ref{sec:screen} and \ref{sec:screen-free}, and every result that depends on
them, operate instead on a stratified sample of 1,628 images spanning 11 sources drawn
from the corpus itself, for the reason given in Section~\ref{sec:screen}.
...
The \emph{multi-source YOLO11n} configuration \cite{yolo11} comprises 2.6\,M
parameters and was trained from COCO initialisation \cite{lin2014coco} on all 13
sources at 640\,px with stochastic gradient descent.

The \emph{multi-source RF-DETR} configuration \cite{rfdetr} is a real-time detection
...
\cite{rousseeuw1993mad}, and $k$ denotes a sensitivity parameter. A value of
...
The median and median absolute deviation replace the mean and standard deviation
because a divergent source inflates both of the latter and thereby masks itself.
...
fence. The cause is a systematic
difference in annotation granularity rather than image difficulty, as
Figure~\ref{fig:scaledist} shows: the flagged source annotates modules at a median
normalised scale of 0.142 against 0.232 across the other sources, with 77\% of its
boxes below 0.20 against 41\% elsewhere. It should be stated plainly that the two
distributions overlap and that many frames in the flagged source are annotated
conventionally, so the divergence is a shift in distribution rather than a categorical
difference. Figure~\ref{fig:examples} shows both ends of that shift within one source.

\begin{figure}[t]
\centering
\includegraphics[width=0.88\columnwidth]{figures/fig_convention_examples.pdf}
\caption{Two frames from the flagged source with their released annotations,
illustrating the range of granularity within a single source. The left frame marks
eleven small internal components as modules; the right marks the six pack-level module
stacks, which is the convention every other source follows. Busbars are annotated
separately in both, so the finer boxes are not mislabelled busbars.}
\label{fig:examples}
\end{figure}
...
them annotates busbars exclusively and contributes none, which the screen surfaces
...
trained. It should be noted that the index detects divergence in annotation \emph{granularity}
specifically. Conventions disagreeing about class boundaries while preserving object
scale would still require the model-based procedure.
...
Table~\ref{tab:control} presents the outcome. The
manipulated source falls from 0.941 to 0.667 and crosses the fence, while no
unmanipulated source moves by more than 0.050 and all but one move by 0.017 or less. The
...
\hline
\multicolumn{4}{l}{\emph{model-based, fence 0.864 / 0.815}} \\
...
\hline
\multicolumn{4}{l}{\emph{training-free, fence 29.02 / 35.24}} \\
...
source under the identical protocol. Table~\ref{tab:arch} and Figure~\ref{fig:arch} present the comparison.
...
\begin{tabular}{lccc}
\hline
Model & Consensus & Mild out. & Diverg. \\
\hline
...
$0.696 \pm 0.003$ and the selected checkpoint $0.461 \pm 0.018$, so the effect is roughly 80
...
\begin{tabular}{lcc}
\hline
Checkpoint & All src. & Module (95\% CI) \\
\hline
Val-selected & $0.461 \pm 0.018$ & 0.552\,[0.471,\,0.626] \\
Fixed-budget & $\mathbf{0.696 \pm 0.003}$ & \textbf{0.735\,[0.659,\,0.808]} \\
\hline
\end{tabular}
\end{table}
...
\noindent On 17 pack families absent from the corpus, drawn from an unlabelled public
collection, the merged-corpus detector attains a mean detection rate of 0.76 and finds
busbars in 16 of the 17, with 1.00 on BMW i4 and Hyundai Ioniq against 0.27 on a Volvo
truck pack, so module geometry transfers to unseen passenger-vehicle packs while
unusual form factors remain the weak axis. Dynamic INT8 quantisation to ONNX reduced
median CPU latency from 56.3\,ms to 44.8\,ms and file size from 6.3\,MB to 3.4\,MB for a
loss of 0.007 mAP@50, measured over 20 images on an Apple M1 processor, giving roughly
22 frames per second without a graphics processor.
...
\cite{yoloworld}, prompted with \texttt{ev battery module} and \texttt{busbar},
...
\cite{groundingdino}. On clean frames the model recovered two of approximately five
...
compositing \cite{ghiasi2021copypaste} combined with a synthetic glare pass was
...
\noindent Two practical implications follow from the results presented above.

Firstly, when a corpus is assembled from heterogeneous sources, per-source
disaggregated reporting is a requirement rather than an option. An aggregate figure
computed across conventions that disagree describes no realisable deployment
condition, and on this corpus it would average a source at 1.000 with one at 0.097.
The proposed screening procedure makes the necessary partition explicit and
reproducible, at the cost of a single evaluation pass over a model that is trained in
any case, or of no training run at all in its geometry-based form.

Secondly, and least expected, the same annotation disagreement that distorts reported
accuracy also distorts model selection, and does so invisibly. A practitioner
following the default behaviour of a standard training framework would have selected
the checkpoint that is worse by 51\% relative, while the training logs showed
monotonically decreasing losses throughout. Validation splits assembled from
heterogeneous corpora therefore warrant the same per-source scrutiny as test splits,
and the screening procedures presented here apply unchanged to them.
