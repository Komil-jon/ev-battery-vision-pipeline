...
\title{Screening Multi-Source Corpora for Annotation Divergence:\\
A Cross-Facility Benchmark for EV Battery Component Detection}
...
\else
\author{
\IEEEauthorblockN{Shiv Ashutosh Katiyar and Komiljon Kosimov}
\IEEEauthorblockA{\textit{Department of Mechanical Engineering,
University of Birmingham}, Birmingham B15 2TT, United Kingdom \\
s.a.katiyar@bham.ac.uk, ~ kxk445@student.bham.ac.uk}
}
\fi

\maketitle
\ifanon\blfootnote{Benchmark definition, provenance and licence record, audit tooling, evaluation code and detector weights are publicly available; the repository URL is withheld during double-blind review.}\else\blfootnote{Benchmark definition, per-source provenance and licence record, audit tooling, evaluation code and detector weights: \url{https://github.com/Komil-jon/ev-battery-vision-pipeline}}\fi
...
materials with elevated supply risk \cite{ec_crm,harper2019}, and around
170\,GWh of batteries are projected to become available for recycling in Europe by
2035 \cite{te2024}.
...
class denotes. Per-source module accuracy spans 1.000 to 0.097, and the collapsing
source annotates at a systematically finer granularity than the rest, its median
module box measuring roughly 60\% the linear size of those in the other sources. The detector
...
\textbf{Relation to prior work.} The single-source specialist evaluated here is the
detector reported by Katiyar \emph{et al.} \cite{katiyar2026mvml}, which addressed detection
...
Two evaluation sets are used and kept distinct. A cross-facility benchmark of 225
images containing 780 instances was drawn from all sources and held out entirely from
training; it supports the single-source comparison of Section~\ref{sec:res-gap} and
nothing else. The screening procedures and every result depending on them operate on a
stratified sample of 1,628 images spanning 11 sources drawn from the corpus itself,
for the reason given in Section~\ref{sec:screen}.
...
The cause is a systematic difference in annotation granularity: the flagged source
annotates modules at a median normalised scale of 0.142 against 0.232 across the other
sources, with 77\% of its boxes below 0.20 against 41\% elsewhere. The distributions
overlap and many of its frames are annotated conventionally, so the divergence is a
shift in distribution rather than a categorical difference. The detector learned the majority convention and applied it consistently, so
...
0.050 and all but one move by 0.017 or less. The flagged set gains exactly one member and
...
so the effect is roughly 80 times the seed spread. On the module class, 500 bootstrap
...
[Online]. Available: \url{https://www.avestia.com/EECSS2026_Proceedings/files/paper/MVML/MVML_125.pdf}
