...
A detection transformer on a frozen self-supervised backbone is shown to confer no
...
transformer with a frozen DINOv2 \cite{dinov2} backbone, fine-tuned on the same
data for 60 epochs at 384\,px. The resolution and epoch budget are the
...
0.680, a loss of 0.091. The defensible claim is correspondingly narrow: a frozen
self-supervised backbone does not detect ordinary modules more accurately, but it
...
a training-free variant flagged the same source with no training at all. A frozen
self-supervised backbone supplies robustness to annotation shift rather than
